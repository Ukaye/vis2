                    ...
                    data.map(x => {
                        ...
                        $('#statementTable').append(
                            `<tr class="item">
                                ...
                                <td>
                                ${(x.is_credit === 1) ? '₦' + (formater(x.amount.split(',').join('')).includes('.') ? formater(x.amount.split(',').join('')) :
                                formater(x.amount.split(',').join('')) + '.00') : ''}
                                </td>
                                <td>
                                ${(x.is_credit === 0) ? '₦' + (formater(x.amount.split(',').join('')).includes('.') ? formater(x.amount.split(',').join('')) :
                                formater(x.amount.split(',').join('')) + '.00') : ''}
                                </td>
                                <td>
                                    ₦${x.balance}
